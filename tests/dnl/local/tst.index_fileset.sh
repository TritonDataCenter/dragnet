...
{
	echo "# dnl query-tree" "$@"
	dnl query-tree "$@" $tmpdir
	echo
}

# Try all the "scan" test cases with an index that should handle them all.
echo "creating indexes" >&2
dnl index-tree -c 'timestamp[date,field=time,aggr=lquantize,step=86400],host,operation,req.caller,req.method,latency[aggr=quantize]' \
    $DN_DATADIR "$tmpdir"
...
dnl index-tree -f '{ "eq": [ "req.method", "GET" ] }' \
...
dnl index-tree -c timestamp[date,field=time,aggr=lquantize,step=60] \
    $DN_DATADIR "$tmpdir"
...
dnl index-tree -c 'timestamp[date,field=time]' /dev/null "$tmpdir"
if [[ -d "$tmpdir" ]]; then

...
FileDataSource.prototype.index = function (args)
{
	...
	scan.pipe(sink);
	scan.on('error', sink.emit.bind(sink, 'error'));
	scan.on('invalid_object', sink.emit.bind(sink, 'invalid_object'));
	scan.on('invalid_record', sink.emit.bind(sink, 'invalid_record'));
	/* XXX */
...
FileDataSource.prototype.query = function (args)
{
	...
	mod_vstream.wrapTransform(rv, 'query op');

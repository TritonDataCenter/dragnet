...
FileSetDataSource.prototype.dataStream = function (findstream)
{
	...
	catstream = mod_vstream.wrapStream(catstream, 'catstream');
...
FileSetDataSource.prototype.scanImpl = function (args)
{
	...
	rv = mod_vstream.wrapTransform(rv, 'scan op');
...
FileSetDataSource.prototype.query = function (args)
{
	...
	rv = mod_vstream.wrapTransform(rv, 'query');
...
FileSetDataSource.prototype.queryImpl = function (args)
{
	...
	aggr = mod_vstream.wrapTransform(aggr, 'aggregator');
	rv = new mod_stream.PassThrough({
	    'objectMode': true,
	    'highWaterMark': 0
	});
	rv = mod_vstream.wrapTransform(rv, 'query op');
...
FileSetDataSource.prototype.index = function (args)
{
	...
	multiplexer = new MultiplexStream({
		...
		dnts = record.pv_value['fields']['__dn_ts'];
		...
		multiplexer.vs_downstreams.push(indexsink);
		return (indexsink);
	    }
	});

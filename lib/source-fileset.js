...
{
	var query, path, finder, aggr, barrier;
	var self = this;
	...
	finder.on('entry', function (filepath, st) {
		...
		barrier.start(filepath);

		queryindex = new QueryIndex({
		    'log': self.fss_log.child({ 'queryindex': filepath }),
		    'filename': filepath
		});

		queryindex.on('error', function (err) {
			err = new VError(err, 'index "%s"', filepath);
			aggr.emit('error', err);
		});
	...
	finder.on('error', function (err) {
		aggr.emit('error', new VError(err, 'find "%s"', path));
	});

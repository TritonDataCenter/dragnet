...
var QueryIndex = require('./index-query');
...
FileSetDataSource.prototype.queryStream = function (findstream, query)
{
	...
	findstream.on('data', function (fileinfo) {
		...
		queryindex.on('ready', function () {
			...
			xform = mod_vstream.wrapTransform(xform, 'Add dn_ts');
...
{
	var path, findstream, datastream, parser, scan;
	 ...
	 * and applying the query to that stream of data.
	 */
	mod_assertplus.string(this.fss_dataroot);
	 ...
	 * the vstream framework to the caller.
	 */
...
FileSetDataSource.prototype.query = function (args)
{
	...
	rv = mod_vstream.wrapTransform(rv, 'Query Passthrough');
...
{
	var path, query, pattern, findstream, querystream, aggr;
...
{
	var index, query, scan, multiplexer, root, barrier;
	var self = this;
	...
	if (args.source == 'raw') {
		...
		if (args.index.ic_before !== null)
			mod_assertplus.string(args.timeFormat,
	                    'args.timeFormat');
	}

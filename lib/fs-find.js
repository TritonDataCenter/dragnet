 ...
 *   stream.
 *
 * - a Statter: an object-mode Transform stream that takes paths as inputs and
 *   produces (path, stat object) tuples
 *
 * - a Traverser: an object-mode Transform stream that takes the output of a
 *   Statter, passes files through directly, and reads dirents in a directory.
 *
 * - an anonymous object-mode Transform stream that passes through regular
 *   files, feeds directories' entries back into the Statter, and ignores
 *   everything else.
 *
 ...
 * Statter and vice versa.  Since we ignore flow control instead, the result is
 * increased memory usage, which should only be observable for very large
 * directories.
 *
...
function FindStream(args)
{
	...
	feedback = mod_streamutil.transformStream({
		...
		if (obj.error || obj.stat.isFile()) {
			fblog.trace('emit "%s": %s', obj.path,
			    obj.error ? 'error' : 'regular file');
			this.push({
			    'path': obj.path,
			    'error': obj.error,
			    'stat': obj.stat
		...
		if (!obj.stat.isDirectory()) {
			fblog.trace('ignore "%s": not file or directory',
			    obj.path);
			setImmediate(callback);
			return;
		}
	 ...
	 * the Statter.  We must also wait for the pipeline to empty out.  We
	 * determine this by sending EOF signals through the pipeline, and we
	 * end the Statter when an EOF signal makes it all the way through
	 * without a recursive write to the Statter.  This feels a little
 ...
 *
 *    output: objects with "path" and either "error" or "stat" fields.
 *
 * Arguments:
 *
 *     log		bunyan-style logger
...
mod_util.inherits(Statter, mod_stream.Transform);

Statter.prototype._transform = function (chunk, _, callback)
{
	var self = this;
 ...
 *
 *    input:  objects with "path" and either "error" or "stat" fields
 *            (as emitted by the Statter)
 *
 *    output: only objects denoting regular files, decorated with either "error"
 *            or "dirents" (for directories).  Directories are emitted as a
 *            "directory" event.
 *
 * Arguments:
 *
...
mod_util.inherits(Traverser, mod_stream.Transform);

Traverser.prototype._transform = function (obj, _, callback)
{
	var self = this;

	if (isEofSignal(obj) || obj.error || !obj.stat.isDirectory()) {
		if (isEofSignal(obj))

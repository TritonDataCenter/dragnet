...
var JOBTPL_EXEC_INDEX0 = [
    ...
    'rm -f $DRAGNET_CONFIG',
    'dn datasource-add input --path=/dev/stdin --data-format=%s %s',
    'dn index-scan --index-config=/assets%s --interval=%s input'
].join('\n');
...
DatasourceManta.prototype.scan = function (args)
{
	...
	job.phase({
	    ...
	    'exec': sprintf(JOBTPL_EXEC_SCAN0, this.ds_format, timefield) +
	        ' ' + this.queryToCliArgs(query, false).join(' ')
	});
	...
	job.phase({
		...
		this.queryToCliArgs(query, true).join(' ')
	});
...
DatasourceManta.prototype.build = function (args, callback)
{
	...
	var inputstream, assetpath, job, timefield;
	...
	if (args.timeBefore !== null || args.timeAfter !== null) {
		if (this.ds_timeformat === null) {
			...
			error = new VError('datasource is missing ' +
			    '"timefield" for "before" and "after" ' +
			    'constraints');
		} else {
			timefield = sprintf(' --time-field=%s',
			    this.ds_timefield);
...
/*
 * [public] Query a set of indexes.  Arguments are the same as scan().
 */
DatasourceManta.prototype.query = function ()
{
	return (mod_dragnet_impl.asyncError(new VError('not yet implemented')));
};

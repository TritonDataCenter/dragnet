...
IndexScanner.prototype.doInitDb = function ()
{
	...
	configpairs.push([ 'index_name', this.is_index.name ]);
	configpairs.push([ 'index_format', this.is_index.format ]);
	configpairs.push([ 'index_filter',
	    JSON.stringify(this.is_index.filter) ]);

	columnpairs = this.is_index.columns.map(function (col) {
		/*
		 * XXX this should be canonicalized earlier in the pipeline so
		 * that we don't have to deal with this here.  The QueryScanner
		 * and other pieces also deal with this separately.
		 */

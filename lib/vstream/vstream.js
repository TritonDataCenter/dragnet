...
function vpushdrop(err, kind)
{
	...
	counter = 'ndrop_' + kind;
	if (!this.vs_counters.hasOwnProperty(counter))
		this.vs_counters[counter] = 0;
	this.vs_counters[counter]++;
 ...
 */
function streamDump(outstream, stream, indentlen)
{
	var i, instr, kind, name, comments, counters;
	var indent = '';
	...
	outstream.write(sprintf('%s%s (%s)\n',
	    indent, name, comments.join(', ')));

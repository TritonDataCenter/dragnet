...
{
	var met, filter_raw, pred, fields_needed, fields_have, datefield;
	var mi, fi, qf, okay;
	...
	for (mi = 0; mi < this.qi_metrics.length; mi++) {
		...
		/*
		 * If the query has before/after constraints, check that we have
		 * a date field associated with this metric.
		 */
		if (query.qc_before !== null || query.qc_after !== null) {
			for (fi = 0; fi < met.qm_params.length; fi++) {
				if (met.qm_params[fi].hasOwnProperty('date'))
					break;
			}

			if (fi == met.qm_params.length) {
				this.qi_log.trace('skipping metric "%s": ' +
				    'no date field', met.qm_label);
				continue;
			}
...
IndexQuerier.prototype.run = function (query)
{
	...
	/* Build WHERE clause. */
	whenfilter = mod_dragnet_impl.queryTimeBoundsFilter(

...
function StreamScan(args)
{
	...
	if (args.timeField) {
		...
		if (args.query.qc_before !== null) {
			mod_assertplus.ok(args.query.qc_after !== null);
			 ...
			 */
			predicate = mod_krill.createPredicate({
			    'and': [ {
			        'ge': [ '__dn_ts', Math.ceil(
				    args.query.qc_after.getTime() / 1000) ]
			    }, {
			        'lt': [ '__dn_ts', Math.ceil(
				    args.query.qc_before.getTime() / 1000) ]
			    } ]
			});
		...
		mod_assertplus.ok(args.query.qc_after === null);
		mod_assertplus.ok(args.query.qc_before === null);
	}

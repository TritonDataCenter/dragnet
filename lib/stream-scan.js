...
function StreamScan(args)
{
	...
	if (args.query.qc_filter) {
		...
		this.ss_filter_query = stream = mod_krill.createPredicateStream(
		    { 'predicate': predicate });
		streams.push(stream);
		mod_vstream.instrumentObject(stream, { 'name': 'user filter' });
		mod_vstream.instrumentTransform(stream,
		    { 'unmarshalIn': false });
		mod_vstream.instrumentPipelineOps(stream);
	}

	if (this.ss_query.qc_synthetic.length > 0) {
		stream = new SyntheticTransformer(this.ss_query);
		streams.push(stream);
		mod_vstream.instrumentObject(stream,
	...
	mod_vstream.instrumentObject(this.ss_skinnerstream,
	    { 'name': 'skinner adapter' });
	mod_vstream.instrumentTransform(this.ss_skinnerstream,
	    { 'unmarshalIn': false });
	mod_vstream.instrumentPipelineOps(this.ss_skinnerstream);
	...
	streams.push(stream);
	mod_vstream.instrumentObject(stream, { 'name': 'aggregator' });
	mod_vstream.instrumentTransform(stream, { 'unmarshalIn': false });
	mod_vstream.instrumentPipelineOps(stream);

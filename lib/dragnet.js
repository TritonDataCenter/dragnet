...
function indexLoad(args)
{
	...
	/*
	 * We only support one interval at the moment.
	 */
	if (rawindex.interval != 'hour')
...
function parseField(b, index, options)
{
	...
	if (typeof (b) != 'string') {
		...
		if (b.hasOwnProperty('date')) {
			if (!b.hasOwnProperty('field'))
				return (new VError('"date" fields must ' +
				    'specify an underlying "field"'));
		}
	...
	if (!rv.hasOwnProperty('date') && !rv.hasOwnProperty('field'))
		rv['field'] = rv['name'];

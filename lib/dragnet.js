...
function datasourceForName(args)
{
	...
	if (dsconfig === null)
		return (new Error('unknown datasource: "%s"', args.dsname));
...
function indexConfig(args, callback)
{
	...
	if (error === null) {
		...
		if (dsconfig === null)
			return (new Error('unknown datasource: "%s"', dsname));
	}
...
function metricsForIndex(args)
{
	...
	metrics = [];
	if (!args.indexConfig) {
		config.metricList(function (metname, mconfig) {
			if (mconfig.m_datasource != dsname)
				return;

 ...
 */
var JOBTPL_EXEC_INDEX = 'mkdir /var/tmp/dn && cat > /var/tmp/dn/data && ' +
    './dragnet/bin/dn index-tree --data-format=%s %s ' +
    '%s /var/tmp/dn /index && cd /index && tar cf /index.tar . && ' +
...
{
	var query, error, job, inputstream, i;
...
MantaDataSource.prototype.query = function (args)
{
	...
	/* XXX commonize with scanJobInit? */
	query = args.query;
	if (query.qc_before !== null && query.qc_after !== null)
		mod_assertplus.string(args.timeFormat, 'args.timeFormat');

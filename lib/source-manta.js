...
var JOBTPL_EXEC_SCAN0 = './dragnet/bin/dnl scan-file --points --data-format=%s';
var JOBTPL_EXEC_QUERY0 = './dragnet/bin/dnl query-file --points %s ' +
    '$MANTA_INPUT_FILE';
/*
...
/*
 * Given a query, return an array of "dnl" command-line arguments that represent
...
/*
 * Given an index, return an array of "dnl" command-line arguments that
 * represent the same index configuration, not including the query-related
 * arguments.
 */
 ...
 *
 *     assetroot	path in Manta for assets
 *
...
function MantaDataSource(args)
{
	...
	this.mds_assetroot = args.assetroot;
...
MantaDataSource.prototype.scanJobInit = function (args)
{
	 ...
	 * for a "dnl scan-file" invocation.  They all use the same dragnet
	 * asset as well.
	 */
	mpipe = '';
...
MantaDataSource.prototype.jobBuilder = function ()
{
	...
	mod_assertplus.string(this.mds_assetroot);
	assetpath = mod_path.join(this.mds_assetroot,
	    sprintf('dragnet-%s.tgz', DRAGNET_VERSION));
	builder = new MantaJobBuilder();
	return ({
	    'assetpath': assetpath,
	    'builder': builder
	});
...
/*
 * Instead of actually running the query as a job, fetch the required indexes to
 * args.cacheroot.
 */
MantaDataSource.prototype.queryFetchOnly = function (args, callback)
{
	var query, inputstream, error;
	var indexroot, cacheroot, log, manta, barrier;

	mod_assertplus.object(args.query, 'query');
	mod_assertplus.string(args.indexroot, 'indexroot');
	mod_assertplus.string(args.cacheroot, 'cacheroot');
	...
	inputstream = this.listInputs(args.indexroot,
	    'by_day/%Y-%m-%d.sqlite', query);
	if (inputstream instanceof Error) {
		setImmediate(function () { callback(inputstream); });
		return;
	}
	...
	error = null;
	barrier.on('drain', function () { callback(error); });

	barrier.start('read stream');
	inputstream.on('end', function () {
		barrier.done('read stream');
	});
	...
	inputstream.on('data', function (input) {
		...
		mod_fs.stat(localpath, function (err, st) {
			...
			log.debug('fetching index "%s"', path);
			mod_vasync.waterfall([
			    function doMkdirp(stepcb) {
				var dirname = mod_path.dirname(localpath);
				mod_mkdirp(dirname, function (suberr) {
					stepcb(suberr);
				});
			    },

			    function doFetch(stepcb) {
				console.error('fetching "%s"', relpath);
				manta.get(path, function (ferr, stream) {
					var out;

					if (ferr) {
						stepcb(new VError(ferr,
						    'fetch "%s"', path));
						return;
					}
					...
					out = mod_fs.createWriteStream(
					    tmppath);
					out.on('error', function (err2) {
						stepcb(new VError(err2,
						    'fetch "%s" read', path));
						return;
					});
			    ...
			    function doRename(stepcb) {
				mod_fs.rename(tmppath, localpath, stepcb);
			    }
			], function (perr) {
				if (perr && !error)
					error = perr;

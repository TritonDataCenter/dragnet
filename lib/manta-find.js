 ...
 *
 *     filter		function to invoke on directories to determine whether
...
if (require.main == module) {
	var log = new (require('bunyan'))({
	    'name': 'manta-find',
	    'level': process.env['LOG_LEVEL'] || 'warn'
	});
	var client = require('manta').createBinClient({ 'log': log });
	process.removeAllListeners('uncaughtException');
	var mf = new MantaFinder({
	    'log': log,
	    'manta': client,
	    'root': '/dap/stor/datasets/cmd',
	    'filter': function () { return (true); }
	});

	mf.on('data', function (chunk) { console.log(chunk.path); });
	mf.on('end', function () { client.close(); });
}

...
exports.indexFindParams = indexFindParams;
...
/*
 * Given information about an index, return parameters used to scan the index.
 * Arguments:
 *
 *     interval		The usual 'day', 'hour', or 'all'
 *
 *     indexpath	Path to the root of the index tree
 *
 *     timeAfter,	Limit the scan to times after timeAfter and before
 *     timeBefore	timeBefore.
 *
 * Returns an object with:
 *
 *     root		Path within the tree to start searching
 *
 *     timeformat	time format string (null for interval "all")
 *
 *     before		start time (null for interval "all")
 *
 *     after		end time (null for interval "all")
 *
 * or returns an Error for unsupported intervals.
 */
function indexFindParams(args)
{
	var root, timeformat, before, after;

	mod_assertplus.object(args, 'args');
	mod_assertplus.string(args.indexpath, 'args.indexpath');
	...
	case 'day':
		root = mod_path.join(args.indexpath, 'by_day');
		timeformat = '%Y-%m-%d.sqlite';
		before = args.timeBefore || null;
		after = args.timeAfter || null;
		break;

	case 'hour':
		root = mod_path.join(args.indexpath, 'by_hour');
	...
	case 'all':
		root = mod_path.join(args.indexpath, 'all');
		timeformat = null;
		before = after = null;
		break;
	...
	default:
		return (new VError(
		    'unsupported interval: "%s"', args.interval));
	}
...
/*
 * Given the JSON-parsed representation of N node-krill filters, return a new
 * filter (in the same representation) that matches only if all of the input
 * filters match.  Any of the input filters may be "null", in which case they
 * are ignored.  If there are no filters provided, then "null" is returned.  (In
 * both cases, "null" is a filter that matches all inputs.)
 */
function filterAnd()
{
	var filters;

	filters = Array.prototype.slice.call(arguments).filter(
